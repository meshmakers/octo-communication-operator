...
| `AuthUri` | Public URI of the identity service issuing access tokens. When set, projected into each workload's Helm values as `authUri`, which the adapter chart renders as `OCTO_ADAPTER__AUTHORITYURL`. Adapters validate bearer tokens against it on secured trigger routes (`FromHttpRequest@2`); when unset such a route rejects every caller because the adapter keeps its `https://localhost:5003` default. |
