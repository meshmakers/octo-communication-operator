...
| `AdditionalValidIssuers` | Extra token issuer values workloads accept on top of `AuthUri` (split-horizon setups, AB#5232). Projected into every workload's Helm values as the `additionalValidIssuers` list; the adapter chart renders `OCTO_ADAPTER__ADDITIONALVALIDISSUERS__<n>`, widening only the issuer STRING comparison (signing keys still come from `AuthUri`'s discovery document). Whitespace-only entries are dropped (an empty entry would be normalized to "/" by the adapter). Canonical use: the local kind dev cluster, where adapters reach identity via `https://host.docker.internal:5003` while callers mint tokens via `https://localhost:5003/`. Env-bindable as `OPERATOR__ADDITIONALVALIDISSUERS__0` etc. |
