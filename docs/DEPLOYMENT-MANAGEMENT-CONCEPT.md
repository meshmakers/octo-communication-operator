...
| HelmChartName | string | no | Helm chart reference (e.g., `oci://registry/charts/energy-dashboard`) |
| HelmChartVersion | string | no | Helm chart version (e.g., `1.0.0`) |
...
| Mode | HelmChartName set? | ImageName set? | Behavior |
|------|-------------------|----------------|----------|
| **Helm** | yes | optional | Operator installs Helm chart with generated values |
| **Image-only** | no | yes | Operator creates Deployment+Service directly (current adapter behavior) |
| **Both** | yes | yes | Helm chart with image override in values |

### Helm Values Generation

The operator generates a Helm values file from the Application attributes:
...
2. Operator:
   - Helm mode: helm upgrade with new values
   - Image-only mode: patch K8s Deployment image
...
2. Operator:
   - Helm mode: helm uninstall
   - Image-only mode: delete Deployment + Service + Ingress
```
